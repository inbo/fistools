...
label_converter <- function(input,
  ...
  if(!soort_column %in% names(input)){
    ...
    ### soort_column is allowed species ? ####
  ...
  if(!labeltype_column %in% names(input)){
    warning(paste0("labeltype_column: ", labeltype_column, " is not a column of input >> checking if its a allowed labeltype"))
    ...
    ### labeltype_column is allowed labeltype ####
    if(labeltype_column %in% standard_lbltype | !unique(temp_input$soort) == "REE"){
      temp_input$labeltype <- labeltype_column
    }else{
      stop("The labeltype_column is not a column of input nor is it a allowed labeltype.")
    }
  }else{
    ...
    nonStanLblType <- sum(is.na(temp_input$labeltype))-sum(temp_input$soort != "REE")

    if(nonStanLblType > 0){
      warning(paste0(nonStanLblType, " rows have a non standard labeltype and are thus removed"))
      temp_input <- temp_input %>%
        dplyr::filter(soort == "REE" & !is.na(labeltype) | soort != "REE")
    }
  }
