...
Extracts spatial features from the OpenStreetMaps server: features that
are extracted are re-classified into broad categories:
...
- proj_bbox:
  ...
  A bbox. The bounding box for the project/ study area for which to
  extract osm features.

- download_features:

  A character. "all" download all features. "polygons", "lines" and
  "points" to download only polygon, line or point features
  respectively. Combinations are also possible (e.g. c("polygons",
  "points")). Default is "all".

- landuse_elements:

  A character. "all" to download all landuse classes. "urban",
  "agriculture", "open", "forest" and "water" to download only landuse
  classes of interest. Combinations are also possible (e.g. c("urban",
  "forest", "water")) Default is "all".

- line_elements:

  A character. "all" to download all line elements. "road", "water" to
  download only roads and rivers, streams etc. respectively. Default is
  "all"

## Value

a named list of 3 sf data frames: osm_polygons, osm_lines, osm_points.
Each sf data frame contains the corresponding geometry types.

## Details

A function to collect custom osm features for a project

dplyr and osmdata are automatically installed when missing from your
system.
